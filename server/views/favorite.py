...
def add_favorite():
    ...
    if user_id or not listing_id:
        return jsonify({"error": "User_id and Listing_id required"}), 404
    
    # prevent duplicate favorites
    existing_favorite = Favorites.query.filter_by(user_id=data['user_id'], listing_id=data['listing_id']).first()
    # If the favorite already exists, return an error
    if existing_favorite:
        return jsonify({"error": "This listing exists in your favorites"}), 400
    # Add the new favorite
    favorite = Favorites(user_id=user_id, listing_id=listing_id, note=note)
